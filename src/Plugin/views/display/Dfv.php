...
namespace Drupal\dfv\Plugin\views\display;
...
/**
 * The plugin that handles an Dependent Field Values display.
 *
 * "dfv" is a custom property, used with
 * \Drupal\views\Views::getApplicableViews() to retrieve all views with a
 * 'Dependent Field Values' display.
 *
 * @ingroup views_display_plugins
 *
 * @ViewsDisplay(
 *   id = "dfv",
 *   title = @Translation("Dependent Field Values"),
 *   admin = @Translation("Dependent Field Values Source"),
 ...
 * )
 */
class Dfv extends DisplayPluginBase {
  ...
  protected function defineOptions() {
    ...
    // 'fields'.
    $options['style']['contains']['type'] = ['default' => 'dfv'];
    $options['defaults']['default']['style'] = FALSE;
    $options['row']['contains']['type'] = ['default' => 'dfv'];
  ...
  public function query() {
    ...
    if ($options['string'] !== '') {
      ...
      if (isset($options['match'])) {
        ...
        if (!empty($style_options['options']["title_field"])) {
          $field_id = $style_options['options']["title_field"];
  ...
  public function validate() {
    ...
    foreach (['title', 'key'] as $key) {
      if (empty($style['options']["{$key}_field"])) {
        $errors[] = $this->t('Display "@display" needs a selected search fields to work properly. See the settings for the DFV list format.', ['@display' => $this->display['display_title']]);
      } else {
        // Verify that the search fields used actually exist.
        $fields = array_keys($this->handlers['field']);

        if (!in_array($style['options']["{$key}_field"], $fields)) {
          $errors[] = $this->t('Display "@display" uses field %field as search field, but the field is no longer present. See the settings for the DFV list format.', ['@display' => $this->display['display_title'], '%field' => $style['options']["{$key}_field"]]);
        }
      }
    }
    return $errors;
